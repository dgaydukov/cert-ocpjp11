...
* New syntax for multi-line string:
  * The starting delimiter is `"""` followed by any number of white spaces + new line character (if you try to use without newline, you get compilation error: `illegal text block open delimiter sequence, missing line terminator`)
  * The ending delimiter is `"""`, no need new line, if you add newline - you add it to your content
  * all escape sequences  `\t`, `\r`, `\n` allowed, and `\` - to denote the absence of new line (this is reverse to `\n`, because here your string is split with `\` but actually not)
  * leading whitespaces removed (all whitespaces from beginning of line to first `{` are removed), yet leading whitespaces inside are preserved
  * trailing whitespaces removed and not part of final string
```java
public class Test {
    public static void main(String args[]) {
        String json = """
...
```
`long` is missed here on purpose. The reasoning is that `long` is used for very large numbers, and probably would never store small values like `byte/short/char`
...
* component fields - list of params passed on record creation, all `final`
  ...
  * each non-canonical constructor must have first line as call to canonical constructor, you can't redefine it, such code won't compile - so when the code reaches second line of such constructor all component fields already set
  ...
  * main difference from class, is that if you provide non-default constructor for class, class will not create canonical constructor, yet for records if you provide any non-canonical constructor, record would still create one by default
```java
record Person (String name, int age) {

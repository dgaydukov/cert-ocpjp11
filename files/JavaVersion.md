...
* [Azul Platform Prime](https://www.azul.com/downloads/#prime) - commercial JVM with codename `Zing` build on top of `HotSpot` optimized for low-latency and high-throughput, including famous C4GC (Continuously Concurrent Compacting Collector) and improved JIT compilation, but available only for Linux. So it's not JDK but only fully optimized JRE with JVM for low-latency systems.
